...
function initialize(){
	...
	const speedSlider = document.getElementById("executeSpeedSlider");
	if (speedSlider) speedSlider.value = geschwindigkeit;
	
	const controlCheckbox = document.getElementById("controlUnitCheckbox");
	if (controlCheckbox) controlCheckbox.checked = false;

	const addressInput = document.getElementById("AddressBusInput");
	if (addressInput) addressInput.addEventListener("keydown",AddressBusInputKeydown);
	
	const dataInput = document.getElementById("DataBusInput");
	if (dataInput) dataInput.addEventListener("keydown",DataBusInputKeydown);
	
	const ramInput = document.getElementById("RamInput");
	if (ramInput) ramInput.addEventListener("keydown",RamInputKeydown);
	
	document.addEventListener("keydown",keyDownHandler);
	document.addEventListener("mousedown",mouseDownHandler);
	window.addEventListener('resize', resize);

	const firstRow = document.getElementById("0");
	if (firstRow) firstRow.style.background = "#00F45D";
	//ladeBildschirm
	loaded = true;
