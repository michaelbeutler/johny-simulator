...
function initialize() {
  ...
  document
    .getElementById("AddressBusInput")
    .addEventListener("keydown", AddressBusInputKeydown); //damit die Entertaste funktioniert
  document
    .getElementById("DataBusInput")
    .addEventListener("keydown", DataBusInputKeydown);
  document
    .getElementById("RamInput")
    .addEventListener("keydown", RamInputKeydown);
...
function fadeOutStartScreen() {
  //für verzögertes ausblenden des Startblidschirms(frühstens nach startScreenFadeOutTime ms  )
...
function resize() {
  ...
  document.getElementById("RamEingabe").style.top =
    document.getElementById(SelectetRamModule).getBoundingClientRect().top -
    RamEingabeHeight / 2 +
    tabelHeight / 2 +
    "px";
...
function getObjectHeight(object) {
  //nimmt ein objekt und gibt die Höhe zurück

  return (
    object.getBoundingClientRect().bottom - object.getBoundingClientRect().top
  );
}
...
function aufnahme() {
  if (recording) {
    ...
    recordingCounter =
      Math.floor(
        CheckNumber(
          parseInt(document.getElementById("aufnahmeZahl").value),
          200,
          0
        ) / 10
      ) * 10; // ignorieren der letzen stelle
    MicroCode[recordingCounter / 10 + 200] =
      document.getElementById("aufnahmeName").value; //speichern des Namens
    document.getElementsByClassName("Mccol1")[recordingCounter].innerText =
      recordingCounter + "   " + MicroCode[recordingCounter / 10 + 200] + ":"; //name im Mc Tabelle einfügen

    for (i = recordingCounter; i < recordingCounter + 10; i++) {
      //zurückseten der Befehle im Mc
      MicroCode[i] = 0;
    ...
    //springen im Mc
    var myElement =
      document.getElementsByClassName("Mccol2")[recordingCounter - 10];
...
function executeProgramm() {
  ...
  if (!halt && alterProgrammzaeler != Programmzaeler) {
...
function AddOpnd(Address) {
  // TODO: should this be enabled in turbo mode?
  ...
  if (MicroCode[high] != undefined && high != 200) {
    document.getElementsByClassName("col4")[Address].innerHTML =
      MicroCode[high];
    document.getElementsByClassName("col5")[Address].innerHTML = parseInt(
      zeroPad(Ram[Address], ramLength + 1).substr(2, ramLength + 1)
    );
  } else {
    document.getElementsByClassName("col4")[Address].innerHTML = "";
...
function nextRamModule() {
  ...
  if (
    document.getElementById(SelectetRamModule).getBoundingClientRect().top +
      tabelHeight / 2 <
    document.getElementById("RamDiv").getBoundingClientRect().bottom
  ) {
    document.getElementById("RamEingabe").style.top =
      document.getElementById(SelectetRamModule).getBoundingClientRect().top -
      RamEingabeHeight / 2 +
      tabelHeight / 2 +
      "px"; //neupositionierung des Peiles für die Rameingabe
  } else {
    document.getElementById("innerRamDiv").scrollTop =
      (SelectetRamModule - 1) * tabelHeight;
    document.getElementById("RamEingabe").style.top =
      document.getElementById(SelectetRamModule).getBoundingClientRect().top -
      RamEingabeHeight / 2 +
      tabelHeight / 2 +
...
function highlightMc(collum) {
  ...
  if (!turboMode) {
    ...
    document.getElementsByClassName("MicroCodeTable")[
      MicroCodeCounter
...
function highlightRamAccess() {
  //übernimmt auch das ändern der unteren Tabelle
  ...
  if (MicroCodeCounter != 1) {
    //nichtanzeigen beim FETCH befehl
