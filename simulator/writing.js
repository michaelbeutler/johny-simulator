...
function writeToRam(Value, Address) {
  Ram[Address] = Value;
  document.getElementsByClassName("col2")[Address].innerHTML =
    zeroPad(Value, ramLength + 1).substr(0, 2) +
    "." +
    zeroPad(Value, ramLength + 1).substr(2, ramLength + 1);
...
function writeToAddressBus(number) {
  ...
  document.getElementById("AddressBus").innerText = zeroPad(
    Addressbus,
    ramLength - 1
  );
}
...
function writeToIns(number) {
  ins = parseInt(number);
  document.getElementById("InsHi").innerText = zeroPad(
    number,
    ramLength + 1
  ).substr(0, 2);
  document.getElementById("InsLow").innerText = zeroPad(
    number,
    ramLength + 1
  ).substr(2, ramLength + 1);
}
...
function writeToMc(number) {
  ...
  document.getElementById("MicoCodeCounter").innerText = zeroPad(
    number,
    ramLength - 1
  );
...
function writeToAcc(number) {
  ...
  document.getElementById("Accumulator").innerText = zeroPad(
    number,
    ramLength + 1
  );
}
...
function writeToPc(number) {
  ...
  document.getElementById("ProgrammCounter").innerText = zeroPad(
    number,
    ramLength - 1
  );
