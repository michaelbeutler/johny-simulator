...
function generateRam() {
  ...
  for (var n = 0; n < ramSize; n++) {
    ...
    document
      .getElementsByClassName("RamCell")
      [n].addEventListener("click", EditRam);
  }
} //ende GenerateRam
...
function GenerateMicroCodeTable() {
  ...
  while (p.firstChild) {
    //entfernen alter einträge(wichtig wenn Mc neu geladen wird)
    p.removeChild(p.firstChild);
  }
  for (i = 200; i < MicroCode.length; i++) {
    document
      .getElementsByClassName("Mccol1")
      [
        (i - 200) * 10
      ].appendChild(document.createTextNode("   " + MicroCode[i] + ":"));

    if (i > 200) {
      //fetch nicht als auswählbaren befehl
      newOption = document.createElement("option");
      Att = document.createAttribute("value");
      Att.value = i - 200;
      newOption.setAttributeNode(Att);
      newOption.appendChild(
        document.createTextNode(zeroPad(i - 200, 2) + ": " + MicroCode[i])
      );
...
function microCodeToText(id) {
  //nimmt eine Microcodeid an und übersetzt diese in den Text wie er in der Tabelle steht
