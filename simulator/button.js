...
function CommandSelectChange() {
  var input = document
    ...
    .value.split(numberDevisionChar);
  document.getElementById("RamInput").value =
    zeroPad(Befehlsauswahl.value, 2) +
    numberDevisionChar +
    zeroPad(input.pop(), ramLength + 1).substr(2, ramLength);
}
function ManuellRam() {
...
function ManuellDb() {
  writeToDb(
    CheckNumber(
      parseInt(document.getElementById("DataBusInput").value),
      (1 + "9".repeat(ramLength)).toString(),
      0
    )
  );
...
function ManuellAB() {
  writeToAddressBus(
    CheckNumber(
      parseInt(document.getElementById("AddressBusInput").value),
      "9".repeat(ramLength - 1).toString(),
      0
    )
  );
}
