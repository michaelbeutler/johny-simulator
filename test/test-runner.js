...
class JohnnyTestRunner {
    ...
    validateInstruction(instruction, address) {
        ...
        if (instruction < 0 || instruction > 99999) {
            errors.push(`Invalid instruction value ${instruction} at address ${address}`);
            return errors;
        }
        ...
        if (opcode >= 1 && opcode <= 6 && (operand < 0 || operand > 999)) {
            errors.push(`Invalid operand ${operand} for opcode ${opcode} at address ${address}`);
        }
        
        // Instructions 07-10 should have operand 000
        if (opcode >= 7 && opcode <= 10 && operand !== 0) {
            errors.push(`Opcode ${opcode} should have operand 000, got ${operand} at address ${address}`);
        }
    ...
    simulateExecution(ram, maxSteps = 10000) {
        ...
        if (!state.halted && state.steps >= maxSteps) {
            throw new Error(`Program exceeded maximum steps (${maxSteps}), possible infinite loop`);
        }
    ...
    runTest(test) {
        ...
        try {
            ...
            if (test.expectedResults) {
                ...
                if (checks.finalAcc !== undefined && executionState.acc !== checks.finalAcc) {
                    result.errors.push(`Expected final accumulator ${checks.finalAcc}, got ${executionState.acc}`);
                }
                
                if (checks.maxSteps !== undefined && executionState.steps > checks.maxSteps) {
                    result.errors.push(`Program took ${executionState.steps} steps, expected max ${checks.maxSteps}`);
                }
                
                if (checks.memoryChanges) {
                    for (const [addr, expectedValue] of Object.entries(checks.memoryChanges)) {
                        const actualValue = executionState.ram[parseInt(addr)];
                        if (actualValue !== expectedValue) {
                            result.errors.push(`Expected memory[${addr}] = ${expectedValue}, got ${actualValue}`);
                        }
                    }
                }
                
                if (checks.shouldHalt !== undefined && executionState.halted !== checks.shouldHalt) {
                    result.errors.push(`Expected halted=${checks.shouldHalt}, got ${executionState.halted}`);
                }
            }
...
if (require.main === module) {
    ...
    const ramFiles = fs.readdirSync(testDir).filter(file => file.endsWith('.ram'));
